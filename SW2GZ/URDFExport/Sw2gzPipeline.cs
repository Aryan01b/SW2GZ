...
namespace SW2GZ.URDFExport
{
    public sealed class Sw2gzPipeline
    {
        ...
        private static string BuildUrdfBodyXml(IReadOnlyList<UrdfLink> links, string pkg)
        {
            ...
            foreach (UrdfLink link in links)
            {
                bodyXml.AppendLine($"  <link name=\"{link.Name}\">");
                ...
                bodyXml.AppendLine($"      <mesh filename=\"package://{pkg}/meshes/{link.VisualMeshFile}\"/>");
                bodyXml.AppendLine("    </geometry></visual>");
                bodyXml.AppendLine("    <collision><geometry>");
                bodyXml.AppendLine($"      <mesh filename=\"package://{pkg}/meshes/{link.CollisionMeshFile}\"/>");
