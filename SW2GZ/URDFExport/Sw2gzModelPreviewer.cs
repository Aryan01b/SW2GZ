...
namespace SW2GZ.URDFExport
{
    public static class Sw2gzModelPreviewer
    {
        ...
        /// <summary>
        /// "Quick preview" entry point — runs the full pipeline against a
        /// synthesized one-link config so the user can see the assembly
        /// rendered as URDF before they ever open Create Robot. Every
        /// non-suppressed top-level component is bundled into a single
        /// `base_link`; no joint inference is attempted. The result is a
        /// faithful "what would Export produce if I put everything in
        /// one rigid link" preview. The synthesized config is NOT
        /// persisted back to the SW document attribute.
        /// </summary>
        public static PreviewResult RunPreviewQuick(SldWorks swApp, ModelDoc2 model)
        {
            if (swApp == null) throw new ArgumentNullException(nameof(swApp));
            if (model == null) throw new ArgumentNullException(nameof(model));
            return RunPreview(swApp, model, BuildQuickPreviewConfig(model));
        }

        /// <summary>
        /// Synthesizes a minimal one-link Sw2gzExportConfig from the live
        /// assembly. Package name derives from the assembly title (or
        /// "robot_preview" if unnamed); every top-level non-suppressed
        /// component lands in a single base_link. Joints stay empty.
        /// </summary>
        public static Sw2gzExportConfig BuildQuickPreviewConfig(ModelDoc2 model)
        {
            if (model == null) throw new ArgumentNullException(nameof(model));

            // Derive a sensible package name from the doc title — fall back
            // to "robot_preview" if the doc is untitled or sanitization
            // produces something empty.
            string raw = model.GetTitle() ?? string.Empty;
            int dot = raw.LastIndexOf('.');
            if (dot > 0) raw = raw.Substring(0, dot);
            string pkg;
            try { pkg = PackageNameSanitizer.Sanitize(raw).Value; }
            catch { pkg = string.Empty; }
            if (string.IsNullOrEmpty(pkg)) pkg = "robot_preview";

            // Collect every non-suppressed top-level component ID for the
            // single base_link. AssemblyDoc.GetComponents(true) returns the
            // top-level slice only (recursive=true would return the entire
            // tree); we want the same scope Create Robot's auto-seed uses.
