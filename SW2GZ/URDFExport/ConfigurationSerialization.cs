...
namespace SW2GZ.URDFExport
{
    ...
    public static class ConfigurationSerialization
    {
        ...
        /// <summary>
        /// Legacy XML deserialization (pre-DataContract SerialNode) is no longer supported in SW2GZ.
        /// Models saved with very old upstream configurations will return a null tree and the user
        /// will need to re-build the link tree.
        /// </summary>
        /// <param name="data">Data string previously deserialized using XMLSerializer (ignored)</param>
        /// <returns>Always null in SW2GZ.</returns>
        private static LinkNode LoadConfigFromStringXML(string data)
        {
            logger.Warn("Legacy pre-DataContract XML configuration detected; SW2GZ no longer " +
                "supports SerialNode deserialization. Rebuild the link tree and re-save.");
            return null;
        }
